...
export let skillMixin = {
  ...
  async _onSkillRoll(statNum, skillNum) {
    return rollSkillCheck(this.actor, statNum, skillNum);
  },
  ...
  async _onSkillDisplay(event) {
    event.preventDefault();
    let section = event.currentTarget.closest(".skill");
    await this.actor.update({ [`system.pannels.${section.dataset.skilltype}IsOpen`]: !this.actor.system.pannels[section.dataset.skilltype + 'IsOpen'] });
  },
